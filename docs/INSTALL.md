...
- **2 hook entries**, from `plugin/hooks/hooks.json`: two `UserPromptSubmit` hooks (`skill-gate.sh`, `ctx-nudge.sh`). The `rg -r`/`-rn`/`-rl` guard (`-r` means `--replace` in ripgrep, not recursive, and the mistake fails silently) now lives in the user's own `PreToolUse` dispatcher, not in this plugin.
