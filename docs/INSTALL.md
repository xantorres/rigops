...
- **1 hook entry**, from `plugin/hooks/hooks.json`: one `UserPromptSubmit` hook (`ctx-nudge.sh`). Two hooks left the plugin for the user's own dispatcher: the `rg -r`/`-rn`/`-rl` guard (`-r` means `--replace` in ripgrep, not recursive, and the mistake fails silently) and the skill-forcing gate.
